...
| UI string rebranding | ✅ | `lib/l10n/app_en.arb` — all user-visible Finamp/Jellyfin strings updated to Naviamp/Navidrome. App tagline, login flow heading, server URL hint, Discord RPC labels, error call-to-action link, and all description strings updated. |
| Playback reporting screen | ✅ | Five Jellyfin-only widgets (Play On toggle, periodic update frequency, queue reporting, Play On stale/reconnection delays) wrapped with `if (!isSubsonic)` in `playback_reporting_settings_screen.dart`. Navidrome users see only the scrobble toggle. |
| Transcoding settings overhaul | ✅ | `FinampTranscodingStreamingFormat.mp3` added (`@HiveField(6)`); `displayName` getter maps enum values to clean labels (MP3, AAC, Opus, FLAC (lossless), Ogg Vorbis). Duplicate HLS container variants (`aacMpegTS`, `vorbisMpegTS`) excluded from UI dropdown but kept in enum for Hive back-compat. Opus set as default streaming format. |
...
| Quick Connect button removed from settings | ✅ | No Navidrome equivalent; `ListTile` and import removed from `settings_screen.dart` |
| Share Server button removed from settings | ✅ | No Navidrome equivalent; `ListTile` and import removed from `settings_screen.dart` |
...
| ~~`drawable-v24`~~ | ~~`ic_launcher_foreground.xml`~~ | Deleted — was a hardcoded Finamp vector that shadowed the PNG on API 24+. Replaced by `drawable/ic_launcher_foreground.png` (manually resized from `assets/icon/icon_foreground.png`). |
